...
library(DESeq2, lib="/gpfs1/cl/ecogen/sw/R/4.5")
library(dplyr)
library(tidyr)
library(ggplot2)
library(scales)
library(ggpubr)
library(vsn, lib="/gpfs1/cl/ecogen/sw/R/4.5")  
library(pheatmap, lib="/gpfs1/cl/ecogen/sw/R/4.5")
#library("vsn")
```
...
Note that we are below the general rule of thumb recomendation of 20M read per sample, We're about 12M
...
Despite lots of zeros across the 119,000 transcripts, we will have an average of 2500 reads mapping per transcript
...
```{r}
hist(apply(countsTableRound,1,mean),xlim=c(0,15000), ylim=c(0,100000),breaks=10000)
```

```{r}
hist(apply(countsTableRound,1,mean),xlim=c(0,3000), ylim=c(0,1000),breaks=10000)
```
...
#Corrects the clumn names to match between the meradata table and the counts matrix
...
Pick up with making the PCA from the tutorial
...
dds$line <- relevel(dds$line, ref = "control")  #aqui se puede cambiar a treatment
...
resG1_CvT <- results(dds, contrast=c("group","controlG1","treatmentG1"), alpha = 0.05) # de la misma manera aqui en los parentesis se pueden cambiar el control o treatment
...
d <-plotCounts(dds, gene="TRINITY_DN151169_c8_g9_i1", intgroup = (c("line","generation")), returnData=TRUE)
d

p <-ggplot(d, aes(x=line, y=count, color=line, shape=generation)) + 
  theme_minimal() + theme(text = element_text(size=20), panel.grid.major=element_line(colour="grey"))
p <- p + geom_point(position=position_jitter(w=0.2,h=0), size=3)# aqui se agrega la linea  scale fill manual (con lo colores)
