...
# 1. Extraer y etiquetar las tablas por generación
gen1_table <- gen1_results$table %>% mutate(Generation = "Gen1")
gen2_table <- gen2_results$table %>% mutate(Generation = "Gen2")
gen3_table <- gen3_results$table %>% mutate(Generation = "Gen3")
gen4_table <- gen4_results$table %>% mutate(Generation = "Gen4")

# 2. Unir todas las tablas
all_go_results <- bind_rows(gen1_table, gen2_table, gen3_table, gen4_table)

# 3. Limpiar y transformar
go_summary <- all_go_results %>%
  select(Generation, Term, classicFisher, Significant) %>%
  mutate(`-log10(p-value)` = -log10(classicFisher)) %>%
  arrange(Generation, classicFisher)



# 5. Ver los primeros términos por generación
go_summary %>% group_by(Generation) %>% slice_max(`-log10(p-value)`, n = 5) %>% ungroup()

```

```{r}
write.csv(go_summary, "GO_comparative_summary.csv", row.names = FALSE)
...
# top 10 terms 
top_terms <- go_summary %>%
  group_by(Generation) %>%
  slice_max(`-log10(p-value)`, n = 10) %>%
  ungroup()

# matrix creation 
heatmap_data <- top_terms %>%
  select(Term, Generation, `-log10(p-value)`) %>%
  pivot_wider(names_from = Generation, values_from = `-log10(p-value)`)

# 
mat <- as.matrix(heatmap_data[,-1])
rownames(mat) <- heatmap_data$Term

# heatmap
pheatmap(mat,
         color = colorRampPalette(c("yellow", "orange", "red", "purple"))(100),
         cluster_rows = FALSE,
         cluster_cols = FALSE,
         main = "GO term significance across generations",
         fontsize_row = 6,
         fontsize_col = 10,
         na_col = "grey90")
...
revigo_all_clean <- bind_rows(revigo_G1, revigo_G2, revigo_G3, revigo_G4)
...
```{r}
revigo_all_clean <- revigo_all_clean %>%
  select(PC_0, PC_1, Frequency, Generation, Value)
...
ggplot(revigo_all_clean, aes(x = PC_0, y = PC_1)) +
  ...
  scale_color_manual(values = c("Gen1" = "red", "Gen2" = "green", "Gen3" = "cyan", "Gen4" = "purple")) +
  theme_minimal(base_size = 12) +
  labs(
    title = "Semantic space of GO terms across generations",
