...
countsTable <- read.csv("counts_matrix.txt", header=TRUE, row.names=1)
dim(countsTable)
...
# Import trait data and filter gene expression based on variance
...
# memory estimate w.r.t blocksize; set maxBlockSize to grater than number of genes to run network construction all at once
...
MEgreenyellow_summary <- summarySE(module_eigengene.metadata, measurevar="MEgreenyellow", groupvars=c("generation","line"))

#Plot a line interaction plot of a particular module eigengene
ggplot(MEgreenyellow_summary, aes(x=as.factor(generation), y=MEgreenyellow, color=line, fill = line, shape = line)) +
  geom_point(size=5, stroke = 1.5 ) +
  geom_errorbar(aes(ymin=MEgreenyellow-se, ymax=MEgreenyellow+se), width=.15) +
...
geneIDs_greenyellow <- module.gene.mapping %>% filter(`bwnet$colors` == 'greenyellow') %>% rownames()

# Filter annotation for module genes -- change the name here
module_GO <- tonsa_go[tonsa_go$geneID %in% geneIDs_greenyellow, ]
